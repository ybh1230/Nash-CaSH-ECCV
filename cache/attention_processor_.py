...
class WanCrossAttnProcessor:
    ...
    def __call__(
        ...
        if query.size(0) < 2:
            ...
            if cache_cross_ouput or reuse_cross_output is None:
                ...
                cached_cross_output = full_cross_states
            else:
                full_cross_states = reuse_cross_output.to(device=query.device, dtype=query.dtype)
                if full_cross_states.shape != window_cross_states.shape:
                    full_cross_states = dispatch_attention_fn(
                        query[1:2],
                        key[1:2],
                        value[1:2],
            ...
            if use_nash:
                hidden_states, authority = nash_equilibrium_mix(
                    ...
                    config=self.nash_config,
                    previous_authority=self._previous_authority,
                )
                log_authority_snapshot(
                    authority=authority,
                    config=self.nash_config,
                ...
                self._previous_authority = authority.detach() if authority is not None else None
            else:
        ...
        if cond and cache_cross_ouput and cached_cross_output is not None:
