...
class WanAttnProcessor:
    ...
    def __call__(
        ...
        cache_cross_attention: Optional[bool] = False,
...
class WanAttention(torch.nn.Module, AttentionModuleMixin):
    ...
    def forward(
        ...
        **kwargs,
        ...
        return self.processor(self, hidden_states, encoder_hidden_states, attention_mask, rotary_emb, reuse_cross_output, cache_cross_ouput, steps, cond, **kwargs)
...
class WanTransformerBlock(nn.Module):
    ...
    def forward(
        ...
        attn_output = self.attn1(norm_hidden_states, None, None, rotary_emb, cache_cross_ouput=cache_cross_ouput, steps=steps, cond=cond)
...
class WanTransformer3DModel(
    ...
    def forward(
        ...
        steps: Optional[int] = None,
        cache_cross_ouput: Optional[bool] = False,
        reuse_cross_output: Optional[list] = None, # Renamed for clarity
        cond: Optional[bool] = False,
        ...
        if torch.is_grad_enabled() and self.gradient_checkpointing:
            for block_idx, block in enumerate(self.blocks):
                attn_for_block = effective_input_maps[block_idx]

                hidden_states, attn_output = self._gradient_checkpointing_func(
                    block, hidden_states, encoder_hidden_states, timestep_proj, rotary_emb, attn_for_block, cache_cross_ouput=cache_cross_ouput, steps=steps, cond=cond
                )
                if attn_output is not None:
                    new_attn_outputs.append(attn_output.to("cuda"))
        else:
            for block_idx, block in enumerate(self.blocks):
                if not cache_cross_ouput and cond:
                    attn_for_block = effective_input_maps[block_idx]
                else:
                    attn_for_block = None
        ...
        if not return_dict:
            return (output, new_attn_outputs)
        
        return Transformer2DModelOutput(sample=output), new_attn_outputs